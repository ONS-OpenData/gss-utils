...
# TODO - at some point we'll use this to switch for other RDFlib output formats
AS_JSON = "json"

def generate_codelist_rdf(dataset_id, df, base_url, output_path, out=AS_JSON):
    """
    ...
    for column_label in df.columns.values:
        ...
        for item in df[column_label].unique():
            ...
            this_code = URIRef(f"{base_url}/def/concept/{dataset_id}/{codelist_id}/{pathify(item)}")
            ...
            # TODO - get it working via rdflib
            triples.append(Triple(
                s=this_code,
                p=SKOS.inScheme,
                o=this_codelist_concept_scheme
        ...
        graph.serialize(output_path, format='json-ld')